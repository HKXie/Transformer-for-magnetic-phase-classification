...
class ViT(nn.Module):
    def __init__(self, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, channels=2, dropout_rate=0.1, token_class=128):
        ...
        # self.pos_embedding = nn.Parameter(torch.randn(1, num_patches + 1, dim))#可学的位置编码
    ...
    def Mask_sequence(self, seq, prob=0.2):
    
        b, s, d=seq.shape
        masked_seq=seq.clone()

        # masked_index=torch.bernoulli(torch.ones(s)*prob)
        # index = []
        # for i, (bernoulli) in enumerate(masked_index):
        #     if bernoulli == 1:
        #         masked_seq[:,i,:]=1.
        #         index.append(i)
        masked_index=torch.randperm(s)[:int(s*prob)]

        for i in masked_index:

            if torch.rand(1)<0.4:
                masked_seq[:,i,:]=torch.randn((b,d))
        ...
        return masked_seq.to(seq.device), masked_index.to(seq.device)

    def forward(self, img, mask=False,classification=True):
        ...
        if mask:
            ...
            x, mask_index = self.Mask_sequence(x,prob=0.4)
            ...
            return self.mlp_seq(x), mask_index#(b,int(hw*p),num_token)# the tokenizer from Transformer
...
def pre_evaluate(model, vae_model, data_loader, loss_history, epoch):
    ...
    with torch.no_grad():
        ...
        for img in data_loader:
            ...
            VAE_token=vae_model.encode(img.clone()).detach()
            # VAE_token=OneHotCategorical(VAE_token[:,pre_index.type(torch.long),:]).sample()
            VAE_token=VAE_token[:,pre_index,:].argmax(dim=-1)
